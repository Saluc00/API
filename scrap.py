...
cursor.execute('CREATE TABLE game (id INTEGER  PRIMARY KEY AUTOINCREMENT, name text);')
cursor.execute('CREATE TABLE player (id INTEGER  PRIMARY KEY AUTOINCREMENT, name text);')
cursor.execute('CREATE TABLE run (id INTEGER  PRIMARY KEY AUTOINCREMENT, rank text, time text, verified text, plateform text, date text, fk_playerID integer, fk_gameID integer, FOREIGN KEY(fk_playerID) REFERENCES player(id), FOREIGN KEY(fk_gameID) REFERENCES game(id));')
conn.commit()
...
for div in soup:
       ...
       req2 = requests.get(f"https://www.speedrun.com/ajax_leaderboard.php?game={href}&verified=1&category=62939&platform=&variable18835=&loadtimes=2&video=&obsolete=&date=").text
       soup2 = BeautifulSoup(req2, 'html.parser').find_all(attrs={'class': 'linked'})
       cursor.execute(f'insert into game (name) values ("{href}")')
       conn.commit()
       idGame = cursor.execute(f'select id from game where name = "{href}"').fetchall()[0][0]
       for run in soup2:
              p = run.find_all('td')
              rank, name, time, verified, plateform, date = getText(p[0]), getText(p[1]), getText(p[2]), getText(p[3]), getText(p[4]), getText(p[5])
              if name not in cursor.execute('select name from player').fetchall():
                     cursor.execute(f'insert into player (name) values ("{name}")')
                     conn.commit()
                     idPlayer = cursor.execute(f'select id from player where name = "{name}"').fetchall()[0][0]
              else:
                     idPlayer = cursor.execute(f'select id from player where name = "{name}"').fetchall()[0][0]
              
              cursor.execute(f'insert into run (rank, time, verified, plateform, date, fk_playerID, fk_gameID) values ("{rank}", "{time}", "{verified}", "{plateform}", "{date}", "{idPlayer}", "{idGame}")')
              conn.commit()
              All.append(Player(getText(p[0]), getText(p[1]), getText(p[2]), getText(p[3]), getText(p[4]), getText(p[5])))
